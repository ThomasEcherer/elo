...
namespace Examples
{
  ...
  /// the corresponding Keyword object.
  /// 
	...
	/// Keyword object where it's Tag member 
	...
	{
    private IXConnection ix;
    private IXServicePortC CONST;
    ...
    private System.Windows.Forms.TextBox edSelectedKeywordText;
		...
		public SelectKeywordDlg(IXConnection ix)
		{
      this.ix = ix;
      this.CONST = ix.CONST;
    ...
    {
      Keyword[] rootKeywords = ix.Ix.checkoutKeywords(null, KeywordC.mbView, 0, LockC.NO);
    ...
    /// <returns></returns>
    public TreeNode[] makeKeywordTreeNodes(Keyword[] keywords)
    {
    ...
    {
      Keyword keyword = ix.Ix.checkoutKeywords(new String[] {kwid}, KeywordC.mbView, 
                                                        CHECKOUT_KEYWORDS_MAX, LockC.NO)[0];
    ...
    private void trKeywords_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
    {
      ...
      {
        Keyword keyword = (Keyword)(trKeywords.SelectedNode.Tag);
        if (keyword != null && keyword.enabled) 
        {
          if (keyword.raw) 
          {
            selectedKeywordText = ix.Ix.cookKeyword(keyword.id);
          } 
          else
          {
    ...
    private void trKeywords_BeforeExpand(object sender, System.Windows.Forms.TreeViewCancelEventArgs e)
    {
      if (e.Node != null) 
      {
        Keyword keyword = (Keyword)e.Node.Tag;

        // it's a dummy node, if the Keyword object has undefined children.
        if (keyword.children == null)
        {
          keyword = ix.Ix.checkoutKeywords(new String[] {keyword.id}, KeywordC.mbView, CHECKOUT_KEYWORDS_MAX, LockC.NO)[0];
          e.Node.Tag = keyword;
          e.Node.Nodes.Clear();
